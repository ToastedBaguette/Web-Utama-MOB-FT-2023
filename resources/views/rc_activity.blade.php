	...
	<section class="vc_row pt-100 pb-60 mt-30">
        <div class="container">
            <header class="fancy-title text-center">
                ...
                <br>
                <p class="my-0 text-white">Silahkan masukkan 6 kode huruf yang sudah dikumpulkan sesuai urutan mendapatkannya</p>
            </header><!-- /.fancy-title -->

            <div class="row transparent-card py-3 mx-3">
                <div class="lqd-column col-md-4 col-md-offset-4">
                    @if (session('status'))
                    <div class="text-center "style="color:#4B8B3B; background-color: #D2E7D6; padding: 1rem; border-radius: 0.5rem">
                        {{ session('status') }}
                    </div>
                    @endif
                    @if (session('error'))
                    <div class="text-center" style="color:#F70000; background-color: #FFE4E2; padding: 1rem; border-radius: 0.5rem">
                        {{ session('error') }}
                    </div>
                    @endif

                    @if(Auth::user()->status == 'maharu')
                    <form action="{{route('tebaksandi')}}" method="POST" class="contact-form contact-form-inputs-underlined contact-form-button-circle">
                    @csrf
                        <div class="row mb-3">
                            <div class="lqd-column col-md-12">
                                <input type="text" id="katasandi" name="katasandi" class="text-white text-center" placeholder="Ketik Kode di sini" style="border: 0; outline: 0; background: transparent; border-bottom: 1px solid white; border-radius: 0;">
                            </div><!-- /.col-md-12 -->
